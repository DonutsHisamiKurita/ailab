...
@media (any-hover: hover) {
  .hoge:hover {
    opacity: 0.5; /* ホバー可能なデバイスでのみ透明度を変更 */
    transition: opacity 0.3s ease; /* 必要に応じてtransitionも */
  }
}

/* タッチデバイスとホバーデバイス共通のスタイルはメディアクエリの外に記述 */
.hoge {
  cursor: pointer;
}
```
...
// 必要に応じてloadイベントやリサイズイベントリスナーを追加
// 例: window.addEventListener('resize', viewportFix);
// 例: window.addEventListener('load', viewportFix);
```