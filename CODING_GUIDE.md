...
- 一貫性 (Consistency): プロジェクト全体、またはチーム全体で同じスタイルを保つことが最も重要です。
- 可読性 (Readability): 他の人が読んだときに理解しやすいコードを書くことを心がけます。
- 保守性 (Maintainability): 将来の変更やデバッグが容易なコードにします。
- シンプルさ (Simplicity): 必要以上に複雑なコードを避けます。
...
- 再代入の予定がない変数には常に `const` を使用します。
- 再代入が必要な変数には `let` を使用します。
- `var` の使用は避けます。（ブロックスコープがないなどの問題があるため）

#### 関数宣言

- アロー関数を優先的に使用するようにしてください。
- 関数の引数が多い場合は、可読性のためにオブジェクトとして渡すことを検討します。

#### コメント

- JSDoc: 関数、クラス、変数などの定義にはJSDoc形式のコメントを使用して、引数、戻り値、型などの情報を記述します。これはドキュメント生成や静的解析に役立ちます。TODOコメントなどは積極的に残すようにしてください。
...
- 上記原則のうち、構文や基本的な書式、一部の命名規則のパターンなど は、プロジェクトに静的解析ツール [eslint](https://eslint.org/) が導入されている場合自動的に確認されます。eslint の出力する Lint エラーや警告は、修正必須の事項として扱ってください。
